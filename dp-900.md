...
- PostgreSQL
  ...
  - single server, flexible server or hyperscale
    ...
    - for big data
  - failure detection
  - pgAdmin tool for DB management# Azure storage for non-relational data
## Blob storage
...
- rows are stored in order of row key (log random access and range queries)# Fundamentals of Azure Cosmos DB
- Scalable NoSQL DB
...
- Cassandra
  ...
  - SQL-based syntax
- Gremlin
  - graph DBs# Fundamentals of Modern Data Warehousing
...
### Notes on Synapse
- Execute permissions must be set on all directories affected by a query, in addition to read permission on files.# Fundamentals of real-time analytics
...
- Uses Kusto Query Language (KQL)
  - Optimized for read
  - Made for DAs
- Especially in Synapse# Fundamentals of Data Visualization
Mainly for DAs
- charts in Excel
- vis. in notebooks
  - Synapse
...
- Link several visualizations
  ...
  - link filters (e.g. sales in Seattle)# Concepts of data analytics
## ELT vs ELT
- ETL
  - simple processing
