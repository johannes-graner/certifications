...
- Prescriptive
  ...
  - decision theory
- Cognitive
  - draw inferences from data and patterns# Fundamentals of Data Visualization
Mainly for DAs
- charts in Excel
- vis. in notebooks
  - Synapse
...
- Link several visualizations
  ...
  - link filters (e.g. sales in Seattle)# Fundamentals of real-time analytics
...
- Uses Kusto Query Language (KQL)
  - Optimized for read
  - Made for DAs
- Especially in Synapse# Fundamentals of Modern Data Warehousing
...
### Notes on Synapse
- Execute permissions must be set on all directories affected by a query, in addition to read permission on files.# Fundamentals of Azure Cosmos DB
- Scalable NoSQL DB
...
- Cassandra
  ...
  - SQL-based syntax
- Gremlin
  - graph DBs# Azure storage for non-relational data
## Blob storage
...
- rows are stored in order of row key (log random access and range queries)# Azure SQL Services and Capabilities
...
- PostgreSQL
  ...
  - single server, flexible server or hyperscale
    ...
    - for big data
  - failure detection
  - pgAdmin tool for DB management# Fundamental Relational Concepts
...
- Index: optimize queries that filter on index column
  - Makes reading fast, writing is slowed# Core data concepts
